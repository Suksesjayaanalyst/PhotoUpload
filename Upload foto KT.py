...
if start2:
    with st.spinner("Processing..."):
        ...
        # FIX: gunakan requests.Session() supaya koneksi dipakai ulang, bukan dibuka-tutup tiap request
        def add_image(img_url, row, session_obj):
            ...
            try:
                ...
                # --- FIX: tutup gambar sumber supaya tidak numpuk di memori ---
                if img is not None:
                    img.close()
            return template
        ...
        # (di image_paths DAN di category_dict) sebelum ditulis ke ZIP di akhir.
        # Ini paling boros memori untuk banyak item. Sekarang kita tulis
        # langsung ke ZIP satu per satu di dalam loop, jadi bytes-nya tidak
        # pernah menumpuk semua di RAM sekaligus.
            ...
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zipf:
                for idx, (_, row) in enumerate(selected_df.iterrows()):
                    ...
                    if not preview_shown:
                        st.image(image_data, caption="First image preview")
                        preview_shown = True
