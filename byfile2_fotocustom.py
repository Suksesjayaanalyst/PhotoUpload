...
if photo:
    ...
    selectprice = st.number_input("Harga", min_value=0)
    selectuom = st.text_input("UOM")
    ...
    if start:
        ...
        def add_text(template, draw, row, font, selectprice):
            ...
            try:
                price = float(selectprice)
                harga_jual = f"Rp. {price:,.0f} / {selectuom}"
            except ValueError:
                harga_jual = f"Rp. {selectprice} / {selectuom}"
